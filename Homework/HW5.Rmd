...
```{r,warning=FALSE, message=FALSE,fig.width=10,fig.height=8}
setwd("C:/Users/xiz933/Desktop/Files/Data Science/2016/xz2315-2016HW5")
...
```{r,warning=FALSE,message=FALSE, fig.width=10,fig.height=8}
# vote - Vote choice
# 1 - Yes 2 - Abstain 3 - No 8 - Absent 9 - Not a member
table(votes$vote)
...
> 1 - Yes or approval for an issue, 2 - Abstain, 3 - No or disapproval for an issue, would provide information about the country's position on an issue; while 8- Absent and 9- Not a member would not.
...
> The column ccode represents countries, it uses a numeric code, called COW(Correlates of War) Country Codes, to represent each country.
...
```{r,warning=FALSE,message=FALSE, fig.width=10,fig.height=8}
library(countrycode)
votes$country <-countrycode(votes$ccode, origin="cown", destination = "country.name")
```
...
**Your answer here**:  
> Votes deemed "important" by the U.S. state Department.

What percent of votes in history were categorized as "important"?
> 6.877% of votes were considered as "important".

```{r,warning=FALSE,message=FALSE, fig.width=10,fig.height=8}
prop.table(table(descriptions$importantvote))
```
...
```{r,warning=FALSE,message=FALSE, fig.width=10,fig.height=8}
votes <- left_join(votes, select(descriptions, -yes,-no,-abstain), by=c("session", "rcid"))
```
...
agreement <- votes %>% 
             ...
             spread(country, vote) %>%
             group_by(year) %>%
             summarise(agreement=mean(USA==Canada,na.rm=TRUE))
 
fit <-lm(agreement~year, data=agreement)
summary(fit)
```
...
> The effect estimate for year on agreement percentage is -0.0069635 (p-value<.005), so the trend predicted by the linear model is decreasing or negative, and also is statistically significant.
...
Reagan administration (1980-1989). 
...
# Local weighted regression (loess) smoothing
library(broom)
fit<-augment(loess(agreement~year, degree=1, span = 0.15, data=agreement)) 
fit %>%
    ggplot(aes(year, agreement)) +
    geom_point(cex=5) + 
    geom_line(aes(x=year, y=.fitted), data=fit, color="red")+
    geom_vline(xintercept = c(1992,2000), col="purple")+
    geom_vline(xintercept = c(1980,1989), col="green")+
    annotate("text", x = 1996, y = 0.35, label = "Clinton administration (1992-2000)",col='purple',size=3)+
    annotate("text", x = 1984, y = 0.35, label = "Reagan administration (1980-1989)",col='green',size=3) +
    ggtitle("Loess Curve for US & Canada Agreement %")
    

```

> During Regan administration, there is a clear negative trend in the agreement 
between the USA and Canada throughout the years; however, during Clinton administration, local weighted regression sugggests a curvilinear-first increasing then decreasing trend.
...
```{r,warning=FALSE,message=FALSE,  fig.width=10,fig.height=7}
#Israel 
agree.notimport <- votes %>% 
             filter(country %in% c("USA", "Israel"), importantvote==0) %>%
             mutate(year = year(date)) %>%
             select(rcid, year, importantvote, country, vote) %>%
             spread(country, vote) %>%
             group_by(year ) %>%
             summarise(agreement=mean(USA==Israel,na.rm=TRUE)) 
         

agree.import <- votes %>% 
             filter(country %in% c("USA", "Israel"), importantvote==1) %>%
             mutate(year = year(date)) %>%
             select(rcid, year, importantvote, country, vote) %>%
             spread(country, vote) %>%
             group_by(year ) %>%
...
fit.notimport<-augment(loess(agreement~year, degree=1, span = 0.15, data=agree.notimport)) 
fit.import<-augment(loess(agreement~year, degree=1, span = 0.15, data=agree.import)) 
 
fit.notimport %>%
    ggplot(aes(year, agreement)) +
    geom_point(cex=5) + 
    geom_line(aes(x=year, y=.fitted), data=fit.notimport, color="red")+
    geom_vline(xintercept = c(1992,2000), col="purple")+
    geom_vline(xintercept = c(1980,1989), col="green")+
    annotate("text", x = 1996, y = 0.35, label = "Clinton administration (1992-2000)",col='purple',size=3)+
    annotate("text", x = 1984, y = 0.35, label = "Reagan administration (1980-1989)",col='green',size=3) +
    ggtitle("Non-Important Issue: Loess Curve for US & Israel Agreement %") 
   
fit.import %>%
    ggplot(aes(year, agreement)) +
    geom_point(cex=5) + 
    geom_line(aes(x=year, y=.fitted), data=fit.import, color="red")+
    geom_vline(xintercept = c(1992,2000), col="purple")+
    geom_vline(xintercept = c(1980,1989), col="green")+
    annotate("text", x = 1996, y = 0.35, label = "Clinton administration (1992-2000)",col='purple',size=3)+
    annotate("text", x = 1984, y = 0.35, label = "Reagan administration (1980-1989)",col='green',size=3) +
    ggtitle("Important Issue: Loess Curve for US & Israel Agreement %") 

# UK
agree.notimport <- votes %>% 
             filter(country %in% c("USA", "UK"), importantvote==0) %>%
             mutate(year = year(date)) %>%
             select(rcid, year, importantvote, country, vote) %>%
             spread(country, vote) %>%
             group_by(year ) %>%
             summarise(agreement=mean(USA==UK,na.rm=TRUE)) 
         

agree.import <- votes %>% 
             filter(country %in% c("USA", "UK"), importantvote==1) %>%
             mutate(year = year(date)) %>%
             select(rcid, year, importantvote, country, vote) %>%
             spread(country, vote) %>%
             group_by(year) %>%
...
fit.notimport %>%
    ...
    annotate("text", x = 1996, y = 0.35, label = "Clinton administration (1992-2000)",col='purple',size=3)+
    annotate("text", x = 1984, y = 0.35, label = "Reagan administration (1980-1989)",col='green',size=3) +
    ggtitle("Non-Important Issue: Loess Curve for US & UK Agreement %") 
...
fit.import %>%
    ...
    annotate("text", x = 1996, y = 0.35, label = "Clinton administration (1992-2000)",col='purple',size=3)+
    annotate("text", x = 1984, y = 0.35, label = "Reagan administration (1980-1989)",col='green',size=3) +
    ggtitle("Important Issue: Loess Curve for US & UK Agreement %") 

#Mexico
agree.notimport <- votes %>% 
             filter(country %in% c("USA", "Mexico"), importantvote==0) %>%
             mutate(year = year(date)) %>%
             select(rcid, year, importantvote, country, vote) %>%
             spread(country, vote) %>%
             group_by(year ) %>%
             summarise(agreement=mean(USA==Mexico,na.rm=TRUE)) 
         

agree.import <- votes %>% 
             filter(country %in% c("USA", "Mexico"), importantvote==1) %>%
             mutate(year = year(date)) %>%
             select(rcid, year, importantvote, country, vote) %>%
             spread(country, vote) %>%
             group_by(year ) %>%
...
fit.notimport %>%
    ...
    annotate("text", x = 1996, y = 0.35, label = "Clinton administration (1992-2000)",col='purple',size=3)+
    annotate("text", x = 1984, y = 0.35, label = "Reagan administration (1980-1989)",col='green',size=3) +
    ggtitle("Non-Important Issue: Loess Curve for US & Mexico Agreement %") 
...
fit.import %>%
    ...
    annotate("text", x = 1996, y = 0.35, label = "Clinton administration (1992-2000)",col='purple',size=3)+
    annotate("text", x = 1984, y = 0.35, label = "Reagan administration (1980-1989)",col='green',size=3) +
    ggtitle("Important Issue: Loess Curve for US & Mexico Agreement %") 

# Cuba
agree.notimport <- votes %>% 
             filter(country %in% c("USA", "Cuba"), importantvote==0) %>%
             mutate(year = year(date)) %>%
             select(rcid, year, importantvote, country, vote) %>%
             spread(country, vote) %>%
             group_by(year ) %>%
             summarise(agreement=mean(USA==Cuba,na.rm=TRUE)) 
         

agree.import <- votes %>% 
             filter(country %in% c("USA", "Cuba"), importantvote==1) %>%
             mutate(year = year(date)) %>%
             select(rcid, year, importantvote, country, vote) %>%
             spread(country, vote) %>%
             group_by(year ) %>%
...
fit.notimport %>%
    ...
    annotate("text", x = 1996, y = 0.35, label = "Clinton administration (1992-2000)",col='purple',size=3)+
    annotate("text", x = 1984, y = 0.35, label = "Reagan administration (1980-1989)",col='green',size=3) +
    ggtitle("Non-Important Issue: Loess Curve for US & Cuba Agreement %") 
...
fit.import %>%
    ...
    annotate("text", x = 1996, y = 0.35, label = "Clinton administration (1992-2000)",col='purple',size=3)+
    annotate("text", x = 1984, y = 0.35, label = "Reagan administration (1980-1989)",col='green',size=3) +
    ggtitle("Important Issue: Loess Curve for US & Cuba Agreement %") 
    

#China
agree.notimport <- votes %>% 
             filter(country %in% c("USA", "China"), importantvote==0) %>%
             mutate(year = year(date)) %>%
             select(rcid, year, importantvote, country, vote) %>%
             spread(country, vote) %>%
             group_by(year ) %>%
             summarise(agreement=mean(USA==China,na.rm=TRUE)) 
         

agree.import <- votes %>% 
             filter(country %in% c("USA", "China"), importantvote==1) %>%
             mutate(year = year(date)) %>%
             select(rcid, year, importantvote, country, vote) %>%
             spread(country, vote) %>%
             group_by(year ) %>%
...
fit.notimport %>%
    ...
    annotate("text", x = 1996, y = 0.35, label = "Clinton administration (1992-2000)",col='purple',size=3)+
    annotate("text", x = 1984, y = 0.35, label = "Reagan administration (1980-1989)",col='green',size=3) +
    ggtitle("Non-Important Issue: Loess Curve for US & China Agreement %") 
...
fit.import %>%
    ...
    annotate("text", x = 1996, y = 0.35, label = "Clinton administration (1992-2000)",col='purple',size=3)+
    annotate("text", x = 1984, y = 0.35, label = "Reagan administration (1980-1989)",col='green',size=3) +
    ggtitle("Important Issue: Loess Curve for US & China Agreement %") 
```
...
* US and Israel had negative trend on the agreement of non-important issues; but almost flat trend on important ones.
* US and UK had negative trends on agreement of both non-important and important issues.
* US and Mexico had a negative trend on the agreement of non-important issues; but positive trend on important ones.
* US and Cuba had a negative trend on the agreement of non-important issues; but positive trend on important ones.
* US and China had negative trends on agreement of both non-important and important issues.

> During Clinton administration:
* US and Israel had positive trend on the agreement of non-important issues;but a curvilinear(first positive then negative) trend on important issues.
* US and UK had negative trends on agreement of both non-important; but curvilinear (first positive and then turning to negative) trend  on important issues.
* US and Mexico had a curvilinear (first positive and then turn to negative) trends on both non-important and important issues.
* US and Cuba had flat trends on the agreement of non-important issues; but a positive trend on the agreement of important issues.
* US and China had a positive-then-negative trend on the agreement of non-important issues; but a almost flat trend on important ones.
...
```{r,warning=FALSE,message=FALSE}
US.d <-c(as.matrix(d)["USA",])
US.d[which.min(US.d[US.d>0])]
US.d[which.max(US.d)]
```

**Your answer here**: 

> Israel is closest to US, CUba is the durthest from US.
...
mydata <- votes %>% 
      filter(country %in% c("USA", "Israel", "UK", "Mexico", "Cuba", "China", "Canada"), importantvote==1) %>%
      filter(vote %in% c(1,3)) %>% #remove "Abstain"
      mutate(vote2=ifelse(vote==1,1,0)) %>% #recode voting results
      select(rcid, country, vote2) %>%
      spread(country, vote2) %>%
      rename(y=USA)
...
library(caret)
set.seed(02138)
TrainIndex  <- createDataPartition(y=mydata$rcid, p=0.8, times=1)
train_set <-slice(mydata, TrainIndex$Resample1)
test_set <-slice(mydata, - TrainIndex$Resample1) 
 
glm_fit <- glm(y~Canada+China+Cuba+Israel+Mexico+UK,data =train_set,family="binomial") 
 
y_hat <- predict(glm_fit, newdata = test_set, type = "response") #default binomial model the default predictions are of log-odds (probabilities on logit scale) and type = "response" gives the predicted probabilities

library(e1071)
tab <- table(pred=round(y_hat), truth=test_set$y)
confusionMatrix(tab)$tab
confusionMatrix(tab)$overall["Accuracy"]
 
```

> The Accuracy is 0.9032258.
...
> Accuracy varies from 88.9% to 96.3%. 
...
mydata<-mutate(mydata, y=as.factor(y))
  
control <- trainControl(method='cv',number=10,p=0.8)
 
Accuracy<-train(data=mydata, 
                y~Canada+China+Cuba+Israel+Mexico+UK,
                method='knn',
                trControl=control,
...
**Your answer here**: 
> We should use k=18 nearest neighbors.
